...
function showOnSCreen(player){
    ...
    <button  id="${player.id}" name="${player.Name}" photo="${player.Photourl}" dob="${player.dob}" birthplace="${player.Birthplace}" matches="${player.Numberofmatches}" score="${player.Score}"  fifties="${player.Fifties}"  centuries="${player.Centuries}" wickets="${player.Wickets}" average="${player.Average}" description="${player.Description}" onClick="edit(event)">Edit</button>
